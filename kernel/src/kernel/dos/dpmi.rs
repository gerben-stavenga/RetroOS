...
pub(super) fn enter_pm_psp_view(dos: &mut thread::DosState) {
...
pub(super) fn restore_rm_psp_view(dos: &mut thread::DosState) {
    if dos.current_psp != PSP_SEL {
        return;
    }
...
pub(super) fn sync_psp_view_for_regs(dos: &mut thread::DosState, regs: &Regs) {
    if regs.mode() == crate::UserMode::VM86 {
