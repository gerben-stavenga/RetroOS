            ...
            .binary_search_by_key(&number, |block| block.number)
        {
            ...
            Err(index) => {
                ...
                self.dirty
                    .try_reserve(1)
                    .map_err(|_| StorageError::new(FsError::OutOfMemory))?;
                self.dirty.insert(index, BlockEdit { number, bytes });
                Ok(index)
            }
        }
    }

    pub fn finish(self) -> Vec<(u64, Vec<u8>)> {
        self.dirty
            .into_iter()
            .map(|block| (block.number, block.bytes))
            .collect()
    }
}
...
impl Storage for BlockOverlay<'_> {
    ...
    fn write(&mut self, offset: u64, input: &[u8]) -> Result<(), StorageError> {
        ...
        while position < end {
            ...
            self.dirty[index].bytes[within..within + count]
