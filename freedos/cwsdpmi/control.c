...
/* Write to QEMU debugcon (port 0xE9) instead of stderr.
   Avoids re-entering DOS/PM INT 21 paths from inside the DPMI host. */
static void dbg_write(char *p, int n)
{
  while(n--) outportb(0xE9, *p++);
}
