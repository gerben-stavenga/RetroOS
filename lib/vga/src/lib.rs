...
//! from the card and does not live in a separate module. The renderer writes
//! only into caller-provided slices; mapped framebuffers, display ownership,
//! firmware modes and publication all remain outside this crate.
//!
...
/// Cursor for one native-width decoded row. Every VGA memory format writes
/// exactly one `u32` per source pixel; presentation owns all scaling and
/// physical framebuffer packing.
struct NativeRow<'a> {
    out: &'a mut [u32],
    x: usize,
}
...
/// Decode one VGA source scanline into `w` native `u32` pixels. `Pal`
/// determines the word's channel encoding; no presentation geometry enters
/// this pass.
...
fn row_planar16(frame: &Frame, sy: usize, pal: &Pal, st: &mut NativeRow<'_>, w: usize, row_bytes: usize) {
    ...
    // into 8 chunky nibbles, rather than re-fetching 4 planes per pixel — which
    // was once essentially the whole cost of EGA/16-colour present.
...
fn row_svga(frame: &Frame, sy: usize, pal: &Pal, st: &mut NativeRow<'_>, w: usize, bpp: u8, pitch: usize) {
    ...
    for x in 0..w {
        ...
        st.put(match bpp {
            ...
            15 => {
                let v = rd16(p);
                pal.fmt.encode(((((v >> 10) & 0x1F) as u32 * 255 / 31) << 16)
                    | ((((v >> 5) & 0x1F) as u32 * 255 / 31) << 8)
                    | ((v & 0x1F) as u32 * 255 / 31))
            }
            16 => {
                let v = rd16(p);
                pal.fmt.encode(((((v >> 11) & 0x1F) as u32 * 255 / 31) << 16)
                    | ((((v >> 5) & 0x3F) as u32 * 255 / 63) << 8)
                    | ((v & 0x1F) as u32 * 255 / 31))
            }
            _ => {
