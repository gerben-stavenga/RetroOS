...
pub fn init_thread(machine: &mut crate::TheArch, threads: &mut [crate::kernel::thread::Thread], tid: usize, data: Vec<u8>, path: &[u8], args: Vec<Vec<u8>>, cmdtail: Vec<u8>, parent_env_data: Vec<u8>, parent_cwd: Vec<u8>) -> Result<(), i32> {
    match detect_format(&data, path) {
        ...
        fmt => {
            ...
            crate::kernel::dos::exec_dos_into(machine, threads, tid, data, is_exe, args, cmdtail, parent_env_data, parent_cwd);
            Ok(())
        }
    }
}
