...
use alloc::vec::Vec;
use client::{
    CMD_CREATE, CMD_MKDIR, CMD_OPEN, CMD_READ, CMD_READDIR, CMD_STAT, CMD_WRITE, HOSTFS_IO_ERROR,
    MAX_READ_CHUNK, MAX_WRITE_CHUNK, RequestWriter, ResponseReader, close, ensure_ready, request,
};
use frame::MAX_BODY;

pub use client::{configure, init, uart_present};
pub use injected::{
    HostBackendHooks, INJECTED_HOSTFS, InjectedHostFs, host_backend_installed, install_host_backend,
};
...
impl Filesystem for HostFs {
    fn open(&self, path: &[u8]) -> Option<Vnode> {
        if !ensure_ready() || path.len() > u16::MAX as usize {
            return None;
        }
        let mut payload = RequestWriter::new();
        if !payload.push_path(path) {
            return None;
        }
    ...
    fn readdir(&self, dir: &[u8], cookie: u64, out: &mut Vec<DirEntry>, max: usize) -> Option<u64> {
        ...
        while out.len() < max {
            if !ensure_ready() || dir.len() > u16::MAX as usize {
                return None;
            }
            let mut payload = RequestWriter::new();
            if !payload.push_path(dir) || !payload.push_u32(index as u32) {
                return None;
            }
            let mut response = [0u8; MAX_BODY];
            ...
            if response_len != 5 + name_len + 9 || name_len > 100 {
                return None;
            }
    ...
    fn dir_exists(&self, path: &[u8]) -> bool {
        ...
        let mut response = [0u8; MAX_BODY];
        let Some(n) = request(CMD_STAT, payload.as_slice(), &mut response) else {
            return false;
        };
        let mut response = match ResponseReader::new(&response[..n], 9) {
            Some(value) => value,
            None => return false,
        };
        let status = match response.read_i32() {
            Some(value) => value,
            None => return false,
        };
        if response.read_u32().is_none() {
            return false;
        }
        let is_dir = match response.read_u8() {
            Some(value) => value,
            None => return false,
        };
        status == 0 && is_dir != 0
    ...
    fn write(&self, handle: u64, offset: u32, data: &[u8]) -> i32 {
        ...
        while total < data.len() {
            ...
            let Some(status) = response.read_i32() else {
                return HOSTFS_IO_ERROR;
            };
